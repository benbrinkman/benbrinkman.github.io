...
function getDivPos(canvas, div, offset) {
    // avoidTest = document.getElementsByClassName("linkBox").getBoundingClientRect();

    var divx = div.getBoundingClientRect().x+offset;
    var divy = div.getBoundingClientRect().y+offset;
    ...
    return {
        x: (divx - rect.left) * scaleX,   // scale mouse coordinates after they have
        y: (divy - rect.top) * scaleY     // been adjusted to be relative to element
...
window.addEventListener('load', function () {
    ...
    window.addEventListener("resize", getAvoidsPos);
    function getAvoidsPos() {
        avoidObjs=[];
        avoidObjsRad = parseFloat(window.getComputedStyle(document.documentElement).getPropertyValue('--bubbleWidth')) / 2;
        for (let obj of document.getElementsByClassName("linkBox")) {
            avoidObjs.push(getDivPos(ctx.canvas, obj, avoidObjsRad));
        }
    ...
    function update() {
        ...
        // for (var i = 0; i < avoidObjs.length; i++) {
        //     ctx.save();
        //     //draw the obj with it's transformation and rotation
        //     ctx.translate(avoidObjs[i].x, avoidObjs[i].y);
        //     circle(avoidObjsRad, "rgb(1,1,1,0.5)");
        //     //            triangle(radius, flock[i].color);
        //     //ctx.fillRect(0,0,radius+5,radius);
        //     ctx.restore();
        // }
    ...
    */
    ...
    var boid = {
        ...
        initBoid: function () {
            ...
            this.color = "rgb(" + 0 + "," + getRndInteger(0, 255) + "," + 0 + ")";
        ...
        flocking: function (boids) {
            ...
            this.applyForce(this.avoidObjects());
        ...
        avoidObject: function () {
            ...
            steer.mult(avoidWeight);
            return steer;
        },
        avoidObjects: function () {
            var steer = Vector(0, 0);
            var count = 0;
            ...
            // var x = document.getElementById("mousePosX").innerHTML;
            // var y = document.getElementById("mousePosY").innerHTML;
            for (let aObj of avoidObjs) {
                var x = aObj.x;
                var y = aObj.y;
                if (x != null) {
                    var avoidObject = Vector(x, y);
                    //var avoidObject = Vector (200.0, 200.0);
                    var d = dist(this.position, avoidObject);// - (avoidObjsRad);
                    if (d < this.myAvoidObjectDistance) {
                        var delta = Vector();
                        delta = this.position.subtract(avoidObject);
                        delta.normalize();
